...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'TOGGLE_OFFLINE') {
    ...
    return true;
  } else if (message.type === 'GET_ALL_OFFLINE') {
    (async () => {
      const offlineTabIds = await getOfflineTabs();
      if (offlineTabIds.length === 0) {
        sendResponse({ tabs: [] });
        return;
      }
      // Fetch full tab info for each offline tab
      const tabInfos = [];
      for (const tabId of offlineTabIds) {
        try {
          ...
          // Tab may have been closed without cleanup
        }
      }
